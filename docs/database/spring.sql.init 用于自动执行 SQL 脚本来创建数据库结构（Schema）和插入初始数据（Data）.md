`spring.sql.init` 是 Spring Boot 2.7+ 版本中引入的一组配置属性，用于**自动初始化数据库（Schema 和 Data）**，替代了旧版
`spring.datasource.schema` 和 `spring.datasource.data` 等过时配置。它提供了更统一、灵活和语义清晰的数据库初始化机制。
...
spring:
  sql:
    ...
    # 根据数据库类型加载对应脚本（如使用 MySQL，则加载 mysql-schema.sql）
...
2. 根据 `platform=mysql` 加载 `mysql-schema.sql` → 创建 `users` 和 `roles` 表
