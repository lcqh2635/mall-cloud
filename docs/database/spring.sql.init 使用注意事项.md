...
- 第一次启动：
    - 执行 `mysql-schema.sql` → 创建 `users` 表
    - 执行 `mysql-data.sql` → 插入 3 条用户记录
- 第二次启动：
    - **再次执行** `mysql-schema.sql` → 如果没有 `IF NOT EXISTS`，会报错（表已存在）
    - **再次执行** `mysql-data.sql` → **再次插入 3 条相同数据** → 可能违反唯一约束（如 `username UNIQUE`）
...
SELECT 'admin',
       'admin@example.com' WHERE NOT EXISTS (SELECT 1 FROM users WHERE username = 'admin');
```
...
-- mysql-data.sql（错误写法）
INSERT INTO users
VALUES (1, 'admin', 'admin@xxx.com');
INSERT INTO users
VALUES (2, 'user1', 'user1@xxx.com');
```
...
```
o.s.b.jdbc.init.ScriptStatementRunner : Executing SQL script from class path resource [mysql-schema.sql]
o.s.b.jdbc.init.ScriptStatementRunner : Executed SQL script from class path resource [mysql-schema.sql] in 45 ms.
o.s.b.jdbc.init.ScriptStatementRunner : Executing SQL script from class path resource [mysql-data.sql]
o.s.b.jdbc.init.ScriptStatementRunner : Executed SQL script from class path resource [mysql-data.sql] in 12 ms.
```
