...
COMMENT
ON TABLE users IS '用户基础信息表（认证专用）';
COMMENT
ON COLUMN users.username IS '登录名，唯一，支持字母、数字、下划线';
COMMENT
ON COLUMN users.password_hash IS 'BCrypt 加密后的密码哈希值';
COMMENT
ON COLUMN users.email IS '邮箱地址，用于找回密码';
COMMENT
ON COLUMN users.status IS '账户状态：ACTIVE=正常, FROZEN=冻结, DELETED=删除';
...
COMMENT
ON TABLE token_blacklist IS 'JWT Token 黑名单表（用于防止重复使用）';
COMMENT
ON COLUMN token_blacklist.token IS '被吊销的JWT Token';
COMMENT
ON COLUMN token_blacklist.reason IS '吊销原因：LOGOUT、RESET_PASSWORD、ADMIN_ACTION';
COMMENT
ON COLUMN token_blacklist.expires_at IS '过期时间（通常为5分钟）';
...
COMMENT
ON TABLE user_profiles IS '用户个人资料表（非认证信息）';
COMMENT
ON COLUMN user_profiles.nickname IS '显示昵称，默认等于username';
COMMENT
ON COLUMN user_profiles.avatar IS '头像URL';
COMMENT
ON COLUMN user_profiles.gender IS '性别';
COMMENT
ON COLUMN user_profiles.phone IS '脱敏手机号（如138****1234）';
COMMENT
ON COLUMN user_profiles.level IS '会员等级：NORMAL、GOLD、PLATINUM、DIAMOND';
COMMENT
ON COLUMN user_profiles.total_spent IS '累计消费金额';
COMMENT
ON COLUMN user_profiles.points IS '积分余额';
...
COMMENT
ON TABLE user_addresses IS '用户收货地址表';
COMMENT
ON COLUMN user_addresses.is_default IS '是否为默认地址';
...
COMMENT
ON TABLE products IS '商品SPU主信息表（商品类型）';
COMMENT
ON COLUMN products.category_path IS '类目路径，如"数码/手机/iPhone"，用于快速筛选';
COMMENT
ON COLUMN products.images IS '多图数组，如["img1.jpg","img2.jpg"]';
...
COMMENT
ON TABLE skus IS '商品SKU销售单元表';
COMMENT
ON COLUMN skus.sku_code IS '唯一编码，如IP15P-128-GRY';
COMMENT
ON COLUMN skus.attributes IS '动态属性，如{"color":"深空灰","storage":"128GB"}';
...
COMMENT
ON TABLE orders IS '订单主表';
COMMENT
ON COLUMN orders.order_no IS '订单编号，格式：ORD20250405123456';
COMMENT
ON COLUMN orders.address_id IS '收货地址ID，下单时快照';
COMMENT
ON COLUMN orders.receiver_name IS '收货人姓名（快照）';
COMMENT
ON COLUMN orders.receiver_phone IS '收货人电话（快照）';
COMMENT
ON COLUMN orders.coupon_id IS '使用的优惠券ID';
COMMENT
ON COLUMN orders.used_points IS '使用的积分数量';
...
COMMENT
ON TABLE order_items IS '订单明细表（商品快照）';
COMMENT
ON COLUMN order_items.attributes IS '商品属性（快照）如{"color":"深空灰","storage":"128GB"}';
...
COMMENT
ON TABLE order_logs IS '订单操作日志表（审计追踪）';
COMMENT
ON COLUMN order_logs.action IS '操作类型：CREATE、PAY、CANCEL、SHIP、DELIVER、REFUND';
COMMENT
ON COLUMN order_logs.operator IS '操作人（USER/ADMIN/SYSTEM）';
...
COMMENT
ON TABLE cart_items IS '购物车项表（MySQL备份，主存储在Redis）';
...
COMMENT
ON TABLE inventories IS '库存主表（真实库存）';
COMMENT
ON COLUMN inventories.warehouse_id IS '仓库编码，如WH-BJ、WH-SH';
COMMENT
ON COLUMN inventories.version IS '乐观锁版本号，防并发冲突';
...
COMMENT
ON TABLE inventory_logs IS '库存操作日志表（审计与对账）';
COMMENT
ON COLUMN inventory_logs.action IS '操作类型：PRE_ALLOCATE、DEDUCT、RELEASE_LOCK、RELEASE_RESERVED、ADJUST';
COMMENT
ON COLUMN inventory_logs.related_type IS '来源类型：CART、ORDER、RETURN、SYSTEM';
COMMENT
ON COLUMN inventory_logs.related_id IS '关联ID（如订单号、购物车ID）';
...
COMMENT
ON TABLE coupon_templates IS '优惠券模板表（规则定义）';
COMMENT
ON COLUMN coupon_templates.code IS '模板编码，如FL_2025';
COMMENT
ON COLUMN coupon_templates.target_users IS '目标用户标签，如["NEW_USER", "VIP"]';
COMMENT
ON COLUMN coupon_templates.exclude_coupons IS '不可叠加的其他券编码列表';
COMMENT
ON COLUMN coupon_templates.products IS '限定商品ID列表';
COMMENT
ON COLUMN coupon_templates.categories IS '限定类目ID列表';
...
COMMENT
ON TABLE coupons IS '优惠券实体表（已发放实例）';
COMMENT
ON COLUMN coupons.code IS '唯一券码，如CUP20250405ABCD';
COMMENT
ON COLUMN coupons.order_id IS '使用该券的订单ID';
...
COMMENT
ON TABLE coupon_usage_logs IS '优惠券使用日志表';
...
COMMENT
ON TABLE waybills IS '运单主表';
COMMENT
ON COLUMN waybills.carrier_code IS '快递公司编码，如SF、JD、ZTO';
COMMENT
ON COLUMN waybills.waybill_no IS '快递公司运单号，如SF123456789CN';
...
COMMENT
ON TABLE logistics_event_logs IS '物流事件日志表';
...
COMMENT
ON TABLE user_preferences IS '用户通知偏好表';
...
COMMENT
ON TABLE notification_logs IS '通知发送日志表';
COMMENT
ON COLUMN notification_logs.event_type IS '触发事件类型，如ORDER_PAID';
COMMENT
ON COLUMN notification_logs.channel_type IS '通道类型：SMS、EMAIL、APP_PUSH、WECHAT_TEMPLATE、INTERNAL_MESSAGE、DINGTALK';
COMMENT
ON COLUMN notification_logs.trace_id IS '链路追踪ID';
...
COMMENT
ON TABLE promotion_templates IS '促销活动模板表';
COMMENT
ON COLUMN promotion_templates.code IS '活动编码';
COMMENT
ON COLUMN promotion_templates.target_users IS '目标用户标签，如["NEW_USER", "VIP"]';
COMMENT
ON COLUMN promotion_templates.exclude_promos IS '不可叠加的其他活动';
COMMENT
ON COLUMN promotion_templates.products IS '限定商品ID列表';
COMMENT
ON COLUMN promotion_templates.categories IS '限定类目ID列表';
...
COMMENT
ON TABLE promotion_usage_logs IS '促销使用日志表';
...
COMMENT
ON TABLE user_behavior_logs IS '用户行为日志表';
COMMENT
ON COLUMN user_behavior_logs.action IS '行为类型：VIEWED、CART_ADDED、ORDER_COMPLETED、FAVORITED';
COMMENT
ON COLUMN user_behavior_logs.source IS '来源：WEB、APP、MINI_PROGRAM';
...
COMMENT
ON TABLE recommendation_logs IS '推荐服务调用日志表';
COMMENT
ON COLUMN recommendation_logs.scene IS '推荐场景：HOME、PRODUCT_DETAIL、CART、ORDER_COMPLETED';
COMMENT
ON COLUMN recommendation_logs.device_type IS '设备类型：WEB、MOBILE、TABLET';
COMMENT
ON COLUMN recommendation_logs.response_time_ms IS '响应耗时（毫秒）';
...
COMMENT
ON TABLE search_logs IS '搜索行为日志表';
COMMENT
ON COLUMN search_logs.keyword IS '搜索关键词';
COMMENT
ON COLUMN search_logs.click_count IS '点击商品数';
...
COMMENT
ON TABLE system_configs IS '系统配置表';
COMMENT
ON COLUMN system_configs.key_name IS '配置键名，如shipping.base_fee';
COMMENT
ON COLUMN system_configs.value_text IS '配置值（JSON字符串或文本）';
